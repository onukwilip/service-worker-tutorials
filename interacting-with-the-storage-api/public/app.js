...
const storage_API = () => __awaiter(void 0, void 0, void 0, function* () {
    if ("storage" in navigator) {
        ...
        if ("persist" in navigator.storage) {
            const isPersist = yield navigator.storage.persist();
            console.log(`Storage ${isPersist ? "IS" : "IS NOT"} persisted`);
        }
    }
});
const get_cached_storage = () => __awaiter(void 0, void 0, void 0, function* () {
    const cache = yield caches.open("imageCache-6");
    let totalSize = 0;
    const returnedMatchResponse = yield cache.matchAll();
    returnedMatchResponse.forEach((matchResponse) => {
        if (matchResponse.headers.has("content-length")) {
            const resSize = parseInt(matchResponse.headers.get("content-length") || "0");
            totalSize += resSize;
            console.log(`Adding size for cache ${matchResponse.url}`);
        }
    });
    console.log(`Total size of cache imageCache-6 is ${totalSize / (1040 * 1040)} MB`);
});
register_sw();
storage_API();
get_cached_storage();
